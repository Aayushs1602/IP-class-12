...
class Minesweeper:
        ...
        for x in range(0, SIZE_X):
            for y in range(0, SIZE_Y):
                ...
                tile = {
                    ...
                    "button": Button(self.frame, image=gfx, height=BTN_HEIGHT, width=BTN_WIDTH),
                ...
                tile["button"].grid(row=x + 1, column=y, padx=BTN_PAD_X, pady=BTN_PAD_Y)  # offset by 1 row for timer
    ...
    def game_over(self, won):
        ...
        if won:
            TIME_LI.append((datetime.now() - self.startTime).total_seconds())
        if res:
            self.restart()
        else:
            self.tk.quit()
